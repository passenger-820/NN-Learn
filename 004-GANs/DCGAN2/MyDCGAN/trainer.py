...
class Trainer(object):
    ...
    def __init__(self, data_loader, config):
        ...
        self.version = config.version                    # [dcgan_10]
    ...
    def train(self):
        ...
        for step in range(start, self.total_step):
            ...
            d_out_real = self.D(real_images) # 让D认识真图
            # 使用铰链损失函数 得到D对真实图片的loss
            ...
            fake_images = self.G(z) # 用于产生假图
            d_out_fake = self.D(fake_images) # 让D去分辨这些假图
            # 使用铰链损失函数 得到D对虚假图片的loss
            ...
            if (step + 1) % self.log_step == 0:
                ...
                print("Elapsed [{}], step [{}/{}], d_loss_real: {:.4f}".format(elapsed, step + 1,self.total_step,d_loss_real.item()))
    ...
    def build_model(self):
        ...
        self.G = Generator(self.imsize, self.z_dim).cuda()
        self.D = Discriminator(self.imsize, self.channels).cuda()
